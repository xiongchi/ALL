...
class fenshiPlan():
    ...
    def drawPlan(self, dataMap, i, epsorpng):
        ...
        try:
            ...
            if ylastprice < 1000:
                # 设置Y轴刻度
                y_data_str = [self.formot_n(y_l, 1),
                              self.formot_n(ylastprice - locator_bottom, 1),
                              self.formot_n(ylastprice, 1),
                              self.formot_n(ylastprice + locator_bottom, 1),
                              self.formot_n(ylastprice + locator_up, 1)]
                # 手动画两条网格虚线
                plt.axhline(ylastprice - locator_bottom, color="#D4D4D4", linewidth=0.5, alpha=0.8, linestyle="--")
                plt.axhline(ylastprice + locator_bottom, color="#D4D4D4", linewidth=0.5, alpha=0.8, linestyle="--")
            else:
                y_data_str = [self.formot_n((ylastprice - locator_up), 4) + '  ',
                              self.formot_n((ylastprice - locator_bottom), 4) + '  ',
                              self.formot_n((ylastprice), 4) + '  ',
                              self.formot_n((ylastprice + locator_bottom), 4) + '  ',
                              self.formot_n((ylastprice + locator_up), 4) + '  ']
                # 手动画两条网格虚线
        ...
        #根据右则股价，计算百分比
        kdbfb_up = self.round_overwrite(float((eval(y_data_str[2]) - eval(y_data_str[0]))) / float(eval(y_data_str[2])) * 100)
        kdbfb_bottom = self.round_overwrite(float((eval(y_data_str[2]) - eval(y_data_str[1]))) / float(eval(y_data_str[2])) * 100)
        plt.figtext(0.91, 0.85, '{:.2f}'.format(kdbfb_up) + "%", color='#FF0000', fontProperties=font_img_)
        plt.figtext(0.91, 0.70, '{:.2f}'.format(kdbfb_bottom) + "%", color='#FF0000',fontProperties=font_img_)
        plt.figtext(0.91, 0.53, "0.00%", color='#334FA0', fontProperties=font_img_)
        plt.figtext(0.91, 0.36, '{:.2f}'.format(-kdbfb_bottom) + "%", color='#1E8726',fontProperties=font_img_)
        plt.figtext(0.91, 0.20, '{:.2f}'.format(-kdbfb_up) + "%", color='#1E8726', fontProperties=font_img_)
    ...
    def round_overwrite(self, num):
        return round(num * 100) / 100.00

    # 截取位数
    def formot_n(self, num, n):
        # 保留3位 从后向前截取n位
        return str('{:.3f}'.format(num))[:-n]
